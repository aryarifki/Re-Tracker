...
export default function RawTablesTab({ ticker, windowDays }: { ticker: string; windowDays: number }) {
  const { data } = useSWR(`/api/bandar/stocks/ticker/raw?window={ticker}/raw?window=ticker/raw?window={windowDays}`, fetcher);
  const flow = data?.flow ?? [];
  const act = data?.activity ?? [];

  const Table = ({ rows, cols, title }: any) => (
    <div className="border border-[var(--line)] rounded-lg bg-[var(--panel)] p-3 overflow-x-auto">
      ...
      {rows.length === 0 ? <p className="text-[var(--muted)] text-sm">Kosong.</p> : (
        <table className="w-full text-[0.72rem]">
          <thead><tr className="text-left text-[var(--muted)] border-b border-[var(--line)]">
            {cols.map((c: string) => <th key={c} className="py-1.5 pr-3">{c}</th>)}
          </tr></thead>
          <tbody>
  ...
  return (
    <div className="space-y-3">
      <Table title="Broker Flow (Ringkasan Harian)" rows={flow}
        cols={["date", "ticker", "foreign_net_broker", "close", "bandar_signal"]} />
      <Table title="Broker Activity (Per Broker per Hari)" rows={act}
